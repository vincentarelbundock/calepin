...
//
// Supported engines: mermaid (mmdc), dot (graphviz), tikz (tectonic), d2.
...
pub fn execute_diagram(
    ...
    // TikZ: compile to PDF with tectonic, then convert to SVG with hayro
    if engine_name == "tikz" {
        ...
        if let Err(e) = convert_pdf_to_svg(&pdf_path, fig_path) {
            results.push(ChunkResult::Error(format!("PDF to SVG conversion failed: {}", e)));
        }
...
/// Convert a PDF file to SVG using hayro.
fn convert_pdf_to_svg(pdf_path: &Path, svg_path: &Path) -> Result<()> {
    use std::sync::Arc;
    ...
    let pdf = Pdf::new(Arc::new(pdf_data))
        .map_err(|e| anyhow::anyhow!("Failed to parse PDF ({}): {:?}", pdf_path.display(), e))?;
    let page = pdf.pages().first()
        .ok_or_else(|| anyhow::anyhow!("PDF has no pages: {}", pdf_path.display()))?;
    let settings = InterpreterSettings::default();
    let svg_data = hayro_svg::convert(&page, &settings);
