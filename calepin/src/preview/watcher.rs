...
/// Watch a directory for `.qmd` and `.yaml` file changes.
/// Calls `on_change` when any matching file is modified.
pub fn watch_dir(dir: &Path, stop: Arc<AtomicBool>, on_change: impl Fn()) -> Result<()> {
    let (tx, rx) = mpsc::channel();
    let mut watcher = notify::recommended_watcher(tx)
        .context("Failed to create file watcher")?;
        ...
        match rx.recv_timeout(Duration::from_millis(200)) {
            Ok(Ok(event)) => {
                let dominated = event.paths.iter().any(|p| {
                    ...
                    matches!(ext, Some("qmd") | Some("yaml") | Some("yml") | Some("bib"))
                });
                if dominated && matches!(event.kind, notify::EventKind::Modify(_) | notify::EventKind::Create(_)) {
                    std::thread::sleep(Duration::from_millis(100));
                    while rx.try_recv().is_ok() {}
