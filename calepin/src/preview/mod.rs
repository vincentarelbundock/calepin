...
/// Preview a website: build _site/, serve with live-reload, watch for changes.
pub fn run_website(config_path: &Path, cli: &Cli) -> Result<()> {
    let config_abs = config_path.canonicalize()
        .with_context(|| format!("Config not found: {}", config_path.display()))?;
    let watch_dir = config_abs.parent().unwrap().to_path_buf();
    ...
    // Initial build
    spinner.set_message("building website...");
    let output_dir = crate::site_builder::build(config_path, true)?;
    site_npm_install(&output_dir)?;
    site_npm_build(&output_dir)?;

    let site_dir = std::env::current_dir()?.join(&output_dir).join("dist");
    ...
    // Watch and rebuild
    let quiet = cli.quiet;
    watcher::watch_dir(&watch_dir, Arc::clone(&stop), || {
        spinner.set_message("rebuilding...");
        let start = std::time::Instant::now();
        match crate::site_builder::build(config_path, true).and_then(|dir| site_npm_build(&dir)) {
            Ok(()) => {
                let elapsed = start.elapsed();
...
/// Run `npm install` in the site output directory if node_modules is missing.
fn site_npm_install(output_dir: &str) -> Result<()> {
    let dir = std::env::current_dir()?.join(output_dir);
...
/// Run `npm run build` in the site output directory.
fn site_npm_build(output_dir: &str) -> Result<()> {
    let dir = std::env::current_dir()?.join(output_dir);
    let shell = std::env::var("SHELL").unwrap_or_else(|_| "/bin/sh".to_string());
    let output = std::process::Command::new(&shell)
        ...
        .current_dir(&dir)
        .output()
        .context("Failed to run site build")?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        anyhow::bail!("Site build failed:\n{}", stderr);
    }
    Ok(())
}
