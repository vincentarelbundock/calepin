...
pub fn handle_new_gibberish(
    ...
    for i in 0..num_files {
        ...
        let title = lorem::lipsum_words(3 + (i * 7) % 4);
        ...
        let heading_titles: Vec<String> = (0..5)
            .map(|h| lorem::lipsum_words(2 + ((i + h) * 5) % 4))
            .collect();
        let sub_titles: Vec<String> = (0..15)
            .map(|s| lorem::lipsum_words(2 + ((i + s) * 3) % 4))
            .collect();
        ...
        for h in 0..5 {
            ...
            for _ in 0..paras_before_subs {
                body.push_str(&lorem::lipsum_paragraphs(1));
                body.push_str("\n\n");
                para_idx += 1;
            }
            ...
            for s in 0..3 {
                ...
                for _ in 0..count {
                    ...
                    for j in 0..sentence_count {
                        ...
                        sentences.push(lorem::lipsum_sentence(len, offset + j * 11));
                    }
                    ...
                    if extras {
                        ...
                        if para_idx % 5 == 1 {
                            let fn_text = lorem::lipsum_sentence(
                                6 + (seed % 5),
                                seed + 3,
                            );
...
fn build_gibberish_table(file_idx: usize, table_idx: usize) -> String {
    ...
    let caption = lorem::lipsum_sentence(5 + (seed % 4), seed + 2);
    out.push_str(&format!(
        "\n: {} {{#{}}} \n",
        caption, label
    ));
