(() => {
  ...
  // Matches theme-init.js's default so the two scripts agree on which URL
  // param to honor when a theme omits `data-calepin-theme-param`: otherwise
  // the init script paints the theme from `?theme=`, and the toggle button's
  // own read (used to sync its label/icon) silently ignores it, flashing.
  const urlParam = (button) => button.dataset.calepinThemeParam || "theme";
