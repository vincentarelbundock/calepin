//! HTML format writer for the unified AST walker.

use comrak::nodes::TableAlignment;

use crate::writers::{FormatWriter, FootnoteStrategy, HeadingAttrs};
#[cfg(test)]
use crate::writers::{WalkOptions, WalkResult, walk_and_render_with_metadata};
...
pub fn markdown_to_html_ast_with_metadata(
    ...
    let fmt_writer = HtmlWriter { standalone };
    walk_and_render_with_metadata(&fmt_writer, markdown, raw_fragments, options)
}
...
mod tests {
    ...
    fn test_heading_with_id() {
        let html = markdown_to_html_ast("# Hello World", &[], false, false, false);
    ...
    fn test_heading_explicit_id() {
        let html = markdown_to_html_ast("## Methods {#sec-methods}", &[], false, false, false);
    ...
    fn test_heading_unnumbered() {
        let html = markdown_to_html_ast("# Appendix {.unnumbered}", &[], true, false, false);
    ...
    fn test_section_numbering() {
        let md = "# First\n\n## Sub\n\n# Second";
        let html = markdown_to_html_ast(md, &[], true, false, false);
    ...
    fn test_shift_headings() {
        let html = markdown_to_html_ast("# Title", &[], false, true, false);
        assert!(html.contains("<h2"), "should shift to h2: {}", html);
    }

    #[test]
    fn test_emphasis() {
        let html = markdown_to_html_ast("*italic* and **bold**", &[], false, false, false);
    ...
    #[test]
    fn test_link() {
        let html = markdown_to_html_ast("[click](https://example.com)", &[], false, false, false);
    ...
    fn test_math_preserved() {
        let html = markdown_to_html_ast("The formula $a^2 + b^2 = c^2$ is important.", &[], false, false, false);
    ...
    fn test_strikethrough() {
        let html = markdown_to_html_ast("~~deleted~~", &[], false, false, false);
        assert!(html.contains("<del>deleted</del>"), "html: {}", html);
    }

    #[test]
    fn test_image() {
        let html = markdown_to_html_ast("![alt text](image.png)", &[], false, false, false);
    ...
    #[test]
    fn test_hr() {
        let html = markdown_to_html_ast("---", &[], false, false, false);
    ...
    fn test_toc_strips_heading_attrs() {
        ...
        let result = markdown_to_html_ast_with_metadata(md, &[], &options, false);
    ...
    fn test_section_counters_chain_across_walks() {
        ...
        let r2 = markdown_to_html_ast_with_metadata("# Third", &[], &opts2, false);
    ...
    fn test_footnote_counter_chains_without_gaps() {
        ...
        let r1 = markdown_to_html_ast_with_metadata(md1, &[], &opts1, false);
        ...
        let r2 = markdown_to_html_ast_with_metadata(md2, &[], &opts2, false);
