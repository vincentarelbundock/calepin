//! AST writers: shared walker + format-specific implementations.
//!
//! A single `walk_ast()` function traverses comrak's node tree and calls
//! methods on a `FormatWriter` trait to produce format-specific output.
...
// ---------------------------------------------------------------------------
// FormatWriter trait (the per-format AST writer)
// ---------------------------------------------------------------------------
...
/// Format-specific output. Each method returns the string(s) to
...
pub trait FormatWriter {
    ...
    /// Called before heading content is rendered (e.g. to write `\section{`).
...
pub fn walk_and_render_with_metadata(
    ...
    let (raw, metadata) = walk_ast(fmt_writer, &protected, options);
    let restored = markers::restore_math(&raw, &math);
    let fmt = fmt_writer.format_name();
...
fn walk_ast(fmt_writer: &dyn FormatWriter, markdown: &str, options: &WalkOptions) -> (String, WalkMetadata) {
    let arena = Arena::new();
    ...
    let footnote_text = match fmt_writer.footnote_strategy() {
    ...
    for edge in root.traverse() {
        match edge {
            comrak::arena_tree::NodeEdge::Start(node) => {
                ...
                write_entering(fmt_writer, &val, &mut out, &mut state, in_list_item);
            }
            comrak::arena_tree::NodeEdge::End(node) => {
                let val = node.data.borrow().value.clone();
                write_leaving(fmt_writer, &val, &mut out, &mut state);
            }
        }
    }
    ...
    if let Some(img) = state.pending_image.take() {
        out.push_str(&fmt_writer.image(&img.url, &img.alt, &ImageAttrs::empty()));
    }

    // Append footnote section if strategy is CollectToSection (unless suppressed)
    if let FootnoteStrategy::CollectToSection = fmt_writer.footnote_strategy() {
        if !state.footnote_defs.is_empty() {
            if options.suppress_footnote_section {
                ...
                out.push_str(&fmt_writer.footnote_section(&state.footnote_defs));
            }
        }
    }
...
fn write_leaving(
    ...
    match val {
        ...
        NodeValue::Document => {}
        // Leaf nodes handled entirely in write_entering
        _ => {}
    }
}
