//! Typst format writer for the unified AST walker.

use comrak::nodes::TableAlignment;

use crate::writers::{FormatWriter, FootnoteStrategy, HeadingAttrs};
#[cfg(test)]
use crate::writers::{WalkOptions, walk_and_render_with_metadata};
...
pub fn markdown_to_typst_with_counter(
    ...
    let fmt_writer = TypstWriter;
    let options = WalkOptions { footnote_counter_start, ..WalkOptions::default() };
    let result = walk_and_render_with_metadata(&fmt_writer, markdown, raw_fragments, &options);
...
impl FormatWriter for TypstWriter {
