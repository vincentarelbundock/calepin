...
#show raw.where(block: true, lang: "bash", theme: auto): it => if _disable-raw-chunk-transforms.get() { it } else { chunk-from-raw-plain("bash", it) }
#show raw.where(block: true, lang: "julia-1.12", theme: auto): it => if _disable-raw-chunk-transforms.get() { it } else { chunk-from-raw-plain("julia-1.12", it) }
...
#include "/.calepin/notebooks/example/source.typ"
