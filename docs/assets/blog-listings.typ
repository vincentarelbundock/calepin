...
#let _read-posts(source) = {
  if type(source) == array or type(source) == dictionary {
    _manifest-posts(source)
...
// Entries from `calepin.pages()` carry author metadata under `meta` and
// resolved values (`path`, `href`, `title`, `pdf`) at the top level. Flatten
// them so listings can be called directly with `calepin.pages()`.
#let _flatten-page-entry(item) = {
  if type(item) != dictionary { return item }
  let meta = item.at("meta", default: none)
  if type(meta) != dictionary { return item }
  let flat = meta
  for key in ("path", "href", "title", "pdf") {
    if item.at(key, default: none) != none {
      flat.insert(key, item.at(key))
    }
  }
  flat
}
...
#let _normalize(item) = {
  let item = _flatten-page-entry(item)
  ...
  let base = if type(item) == dictionary { item } else { (:) }
  base + (
    path: path,
    href: href,
    title: title,
    ...
    display: _get(item, "display", default: true),
  )
}
