...
let watchPanel = null;
let watchOutput = null;
let watchPdfFileWatcher = null;
let watchPageSyncFileWatcher = null;
let watchInput = null;
let watchPageSyncPath = null;
let watchPageSyncEntries = [];
...
class PreviewManager {
    constructor(context, output) {
        ...
        this.diagnostics = vscode.languages.createDiagnosticCollection("calepin");
        this.subscriptions = [];
        this.subscriptions.push(this.diagnostics, vscode.workspace.onDidChangeTextDocument((event) => {
    ...
    async openPreview(uri) {
        ...
        const session = new PreviewSession(this.context, document, binary, outputPath, env, this.output, this.diagnostics, () => this.sessions.delete(key));
        this.sessions.set(key, session);
        await session.start();
    }
    async restartPreview(uri) {
        const input = await resolveTypstFile(uri);
        if (!input)
            return;
...
class PreviewSession {
    constructor(context, document, binary, outputPath, processEnv, output, diagnostics, onDispose) {
        this.context = context;
        ...
        this.stdoutBuffer = "";
        this.version = 0;
        this.latestSentVersion = 0;
        this.latestRenderedVersion = 0;
    ...
    async start() {
        this.panel = vscode.window.createWebviewPanel("calepinPreview", `Calepin Preview: ${path.basename(this.document.uri.fsPath)}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
        ...
        this.panel.webview.html = pdfWatchWebviewHtml(this.context, this.panel.webview, this.outputPath);
        this.startProcess();
    ...
    scheduleSnapshot() {
        ...
        const debounceMs = cfg.get("preview.debounceMs", 250);
        this.debounceTimer = setTimeout(() => {
            this.debounceTimer = null;
            this.sendSnapshotNow(false);
        }, debounceMs);
    }
    sendSnapshotNow(exec = false) {
    ...
    startProcess() {
        const cfg = vscode.workspace.getConfiguration("calepin");
        const format = cfg.get("preview.format", "pdf");
        const noExec = cfg.get("preview.noExec", true);
        const extraTypstArgs = cfg
            .get("preview.extraTypstArgs", [])
        ...
        const proc = (0, child_process_1.spawn)(this.binary, args, spawnOptions);
        this.process = proc;
        ...
        proc.on("exit", (code) => {
            if (this.disposed || this.process !== proc)
                return;
            this.process = null;
    ...
    handleCompiled(message) {
        const version = typeof message.version === "number" ? message.version : 0;
        try {
            if (version < this.latestSentVersion || version < this.latestRenderedVersion)
                return;
            ...
            const preserveScroll = vscode.workspace
                ...
                .get("preview.preserveScroll", true);
    ...
    handleError(message) {
        ...
        try {
            if (version < this.latestSentVersion)
                return;
            this.applyDiagnostics(message.diagnostics ?? []);
    ...
    finishCompile() {
        this.compileInFlight = false;
        if (!this.pendingSnapshot || this.disposed)
            return;
...
function toVscodeDiagnostic(item) {
    ...
    const range = new vscode.Range(startLine, startCol, Math.max(startLine, endLine - 1), Math.max(startCol + 1, endColumn - 1));
    const diagnostic = new vscode.Diagnostic(range, item.message ?? "Calepin preview error", protocolSeverity(item.severity));
    diagnostic.source = item.source ?? "calepin";
    return diagnostic;
}
function positiveProtocolInteger(value) {
    return typeof value === "number" && Number.isInteger(value) && value > 0 ? value : null;
}
function protocolSeverity(value) {
    switch (value) {
        case "warning":
...
async function livePreviewOutputPath(input) {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("calepin.watch", (uri) => runWatch(context, uri)), vscode.commands.registerCommand("calepin.compile", (uri) => runCompile(context, uri)), vscode.commands.registerCommand("calepin.stop", () => runStop(context)), vscode.commands.registerCommand("calepin.new", () => runNew(context)), vscode.commands.registerCommand("calepin.openPreview", (uri) => previewManager?.openPreview(uri)), vscode.commands.registerCommand("calepin.restartPreview", (uri) => previewManager?.restartPreview(uri)), vscode.commands.registerCommand("calepin.stopPreview", (uri) => previewManager?.stopPreview(uri)), vscode.commands.registerCommand("calepin.refreshExecution", (uri) => previewManager?.refreshExecution(uri)), vscode.window.onDidChangeTextEditorSelection((event) => scheduleEditorToPdfSync(event.textEditor)), vscode.window.onDidChangeActiveTextEditor((editor) => {
        if (editor)
            scheduleEditorToPdfSync(editor);
    }));
}
function deactivate() {
    stopWatch();
...
async function runWatch(context, uri) {
    const input = await resolveTypstFile(uri);
    if (!input)
        return;
    ...
    if (!binary)
        return;
    const cfg = vscode.workspace.getConfiguration("calepin");
    const format = cfg.get("watchFormat", "pdf");
    const typstArgs = cfg.get("watchTypstArgs", []).filter((arg) => arg.length > 0);
    const env = await calepinProcessEnv(input);
    startWatchProcess(context, binary, input, format, typstArgs, env);
}
async function runCompile(context, uri) {
    const input = await resolveTypstFile(uri);
    if (!input)
        return;
    const binary = await findBinary(context);
    ...
    if (!output)
        return;
    runInTerminal("Calepin Typst Compile", binary, ["compile", input.fsPath, output.fsPath, "--format", format], workspaceCwd(input), await calepinProcessEnv(input));
}
async function pickCompileFormat() {
    const picked = await vscode.window.showQuickPick([
        { label: "PDF", description: "pdf", format: "pdf" },
        { label: "HTML", description: "html", format: "html" },
    ], {
...
async function pickCompileOutput(input, format) {
...
async function runStop(context) {
    stopWatch();
    const binary = await findBinary(context);
    if (!binary)
        return;
    const activeInput = vscode.window.activeTextEditor?.document;
    const stopInput = activeInput?.uri.scheme === "file" && activeInput.uri.fsPath.endsWith(".typ")
        ? activeInput.uri
        : watchInput;
    if (stopInput) {
        runInTerminal("Calepin Typst Stop", binary, ["stop", stopInput.fsPath], workspaceCwd(stopInput), await calepinProcessEnv(stopInput));
        return;
    }
    ...
    for (const folder of workspaceFolders) {
        runInTerminal("Calepin Typst Stop", binary, ["stop"], folder.uri.fsPath, await calepinProcessEnv(folder.uri));
    }
}
...
function runInTerminal(name, binary, args, cwd, env) {
    const terminal = vscode.window.createTerminal({ name, cwd, env });
    terminal.show();
    terminal.sendText([binary, ...args].map(shellQuote).join(" "));
}
function startWatchProcess(context, binary, input, format, typstArgs, processEnv) {
    stopWatch();
    watchInput = input;
    ...
    const spawnOptions = processEnv
        ...
        : { cwd };
    const proc = (0, child_process_1.spawn)(binary, args, spawnOptions);
    watchProcess = proc;
    let opened = false;
    if (format === "pdf") {
        opened = true;
    ...
    proc.on("error", (error) => {
        ...
        vscode.window.showErrorMessage("Failed to start Calepin Typst Watch. See the Calepin Typst Watch output for details.");
    });
    proc.on("exit", (code) => {
...
function startPdfOutputWatcher(output) {
    watchPdfFileWatcher?.dispose();
    const pattern = new vscode.RelativePattern(vscode.Uri.file(path.dirname(output.fsPath)), path.basename(output.fsPath));
    const watcher = vscode.workspace.createFileSystemWatcher(pattern);
    watcher.onDidCreate(reloadPdfPreview);
    watcher.onDidChange(reloadPdfPreview);
    watchPdfFileWatcher = watcher;
}
function startPageSyncWatcher(input) {
    watchPageSyncFileWatcher?.dispose();
    watchPageSyncPath = vscode.Uri.file(defaultPageSyncPath(input.fsPath));
    const pattern = new vscode.RelativePattern(vscode.Uri.file(path.dirname(watchPageSyncPath.fsPath)), path.basename(watchPageSyncPath.fsPath));
    const watcher = vscode.workspace.createFileSystemWatcher(pattern);
...
async function reloadPageSyncMap() {
    if (!watchPageSyncPath)
        return;
    try {
...
function parsePageSyncEntries(document) {
    ...
    return document.entries
        ...
        if (!label || !file || !line || !page)
            return null;
        return { label, file, line, page };
    })
        .filter((entry) => entry !== null)
        .sort((left, right) => left.file.localeCompare(right.file) || left.line - right.line || left.page - right.page);
}
...
function scheduleEditorToPdfSync(editor) {
    if (!watchPanel || !watchInput || watchPageSyncEntries.length === 0)
        return;
    if (editor.document.uri.scheme !== "file")
        return;
    if (!hasPageSyncEntriesForFile(editor.document.uri))
        return;
    if (editorSyncTimer) {
...
function syncEditorToPdf(editor) {
...
function hasPageSyncEntriesForFile(uri) {
    return watchPageSyncEntries.some((entry) => sameFsPath(uriForPageSyncEntry(entry).fsPath, uri.fsPath));
}
function nearestEntryForSourceLine(uri, line) {
    const entries = watchPageSyncEntries.filter((entry) => sameFsPath(uriForPageSyncEntry(entry).fsPath, uri.fsPath));
    if (entries.length === 0)
        return null;
    let best = entries[0];
...
function uriForPageSyncEntry(entry) {
...
function showPdfWatchWebview(context, input, output) {
    const mediaRoot = vscode.Uri.joinPath(context.extensionUri, "media", "pdfjs");
    if (!watchPanel) {
        watchPanel = vscode.window.createWebviewPanel("calepinWatch", `Calepin Typst Watch: ${path.basename(input.fsPath)}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
            retainContextWhenHidden: true,
            localResourceRoots: [
                mediaRoot,
        ...
        watchPanel.webview.onDidReceiveMessage((message) => {
            if (message.type === "timing" && message.message) {
...
function showWatchWebview(url, input) {
    if (!watchPanel) {
        watchPanel = vscode.window.createWebviewPanel("calepinWatch", `Calepin Typst Watch: ${path.basename(input.fsPath)}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
            retainContextWhenHidden: true,
...
function watchWebviewHtml(webview, url) {
    const csp = [
        `default-src 'none'`,
...
function pdfWatchWebviewHtml(context, webview, output) {
    ...
    const workerUri = webview.asWebviewUri(vscode.Uri.joinPath(mediaRoot, "build", "pdf.worker.min.mjs"));
    const viewerJsUri = webview.asWebviewUri(vscode.Uri.joinPath(mediaRoot, "web", "pdf_viewer.mjs"));
    const viewerCssUri = webview.asWebviewUri(vscode.Uri.joinPath(mediaRoot, "web", "pdf_viewer.css"));
    const csp = [
        `default-src 'none'`,
...
function formatCommand(binary, args, env) {
...
function defaultPageSyncPath(input) {
    const parsed = path.parse(input);
