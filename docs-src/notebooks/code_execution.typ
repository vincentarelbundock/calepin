...
#table(
  ...
  [fig-device-format], [`"svg"`], [Format for figure files written by the engine: `svg`, `png`, `jpeg` (alias `jpg`), or `pdf`. PDF figures render only in paged output; on the HTML target a `pdf` figure fails the render with an error naming the chunk. Diagram engines always emit `svg` regardless of this setting.],
